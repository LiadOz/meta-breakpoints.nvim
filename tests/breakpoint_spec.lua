...
describe("test breakpoints without persistence", function()
  ...
  --it("setup meta breakpoint", function()
    ...
    --local bp = bps.get_all_breakpoints()[1]
    ...
    --bp = bps.get_all_breakpoints()[1]
    --assert.equals(1, signs.get_sign_id_data(bp.sign_id, bp.bufnr).lnum)
    --assert.are.same({ { buffer, 1 } }, get_dap_breakpoints())

    --bps.toggle_meta_breakpoint({}, { bufnr = buffer, lnum = 1 })
    --assert.are.same({}, bps.get_all_breakpoints())
  ...
  --it("checks multiple buffers", function()
    ...
    --local breakpoints = bps.get_all_breakpoints()
...
describe("test breakpoints with persistence", function()
  ...
  local function save_breakpoints(bufnr)
    ...
    persistence.update_buf_breakpoints(bufnr, true, true, function()
      coroutine.resume(co)
    end)
    coroutine.yield()
  end
  ...
  local function get_breakpoints_for_buffer(bufnr)
    local result = {}
    for _, breakpoint_data in pairs(bps.get_all_breakpoints()) do
  ...
  it('checks load per buffer', function()
    ...
    local bp_before_saving = bps.get_all_breakpoints()[2]
    save_breakpoints(second_buffer)
    assert(#bps.get_all_breakpoints() == 2)
    buffer = assert_saved_breakpoints(buffer)

    vim.print(bps.get_all_breakpoints())
    local all_breakpoints = bps.get_all_breakpoints()
  ...
  it('checks removed breakpoint does not persist', function()
    print('placing the first breakpoint')
    bps.toggle_meta_breakpoint({}, { bufnr = buffer, lnum = 3 })
    print('finished placing the first breakpoint')
    buffer = assert_saved_breakpoints(buffer, {bufnr = buffer})
    assert.equals(1, #bps.get_all_breakpoints())
    print(' ---- placing the second breakpoint ----')
    bps.toggle_meta_breakpoint({}, { bufnr = buffer, lnum = 3 })
    print('in test starting to save breakpoints')
    save_breakpoints()
    print('in test finished saving breakpoints')
    print(' ---- finished placing the second breakpoint ----')
    assert.equals(0, #bps.get_all_breakpoints())
    load_breakpoints()
    assert.equals(0, #bps.get_all_breakpoints())
  end)
